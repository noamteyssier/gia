...
use crate::{
    io::NamedInterval,
    types::{Bed3Set, NumericBed3, SplitTranslater, TranslateGroup},
};
...
pub fn read_bed3_set<R: Read>(
    reader: R,
    named: bool,
) -> Result<(Bed3Set, Option<SplitTranslater>)> {
    if named {
...
fn read_bed3_set_named<R: Read>(reader: R) -> Result<(Bed3Set, SplitTranslater)> {
...
fn convert_bed3_set<R: Read>(reader: R, translater: &mut SplitTranslater) -> Result<Bed3Set> {
    let mut reader = build_reader(reader);
    ...
    while reader.read_byte_record(&mut raw_record)? {
        ...
        translater.add_name(record.name, TranslateGroup::Chr);
        let chr_int = translater
            .get_idx(record.name, TranslateGroup::Chr)
            .unwrap();
